...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        ## Initialize the velocities tensor be dictionary with key as number and 
        ## value as tensor of zeros with same shape as the parameter tensor
        self.velocities = { i: torch.zeros_like(p) for i, (p, _) in enumerate(self.params) }
        self.index = 0
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            self.velocities[self.index] = self.momentum * self.velocities[self.index] - self.learn_rate * dp
            dp = dp + self.reg * p
            p += self.velocities[self.index]
            self.index += 1
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        ## Initialize the gradient square tensor be dictionary with key as number and
        ## value as tensor of zeros with same shape as the parameter tensor
        self.grad_square = { i: torch.zeros_like(p) for i, (p, _) in enumerate(self.params) }
        self.index = 0
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            self.grad_square[self.index] = self.decay * self.grad_square[self.index] + (1 - self.decay) * dp**2
            dp = dp + self.reg * p
            p -= self.learn_rate * dp / (torch.sqrt(self.grad_square[self.index]) + self.eps)
            self.index += 1
        self.index = 0
            # ========================
