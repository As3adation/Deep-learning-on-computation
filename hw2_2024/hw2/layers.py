...
class LeakyReLU(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        f_1 = self.alpha * x # incase x is negative
        f_2 = x # in case x is positive
        out = torch.max(f_1, f_2) 
        # ========================
    ...
    def backward(self, dout):
        ...
        dx = torch.where(x > 0, one, self.alpha * one)
        dx = dx * dout   
...
class ReLU(LeakyReLU):
    ...
    def __init__(self):
        # ====== YOUR CODE: ======
        # as we saw in the lectures the leaky relu add support for negative values
        # so for negative value in ReLU alpha value is 0
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # manually calculate the derative of the function 1/(1+exp(-x)) from calculas
        dx = dout * torch.exp(-self.grad_cache["x"]) / (1 + torch.exp(-self.grad_cache["x"])) ** 2
        # ========================
...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        # manually calculate the derative of the function (exp(x) - exp(-x)) / (exp(x) + exp(-x)) f
        
        dx = dout *4 / (torch.exp(self.grad_cache["x"]) + torch.exp(-self.grad_cache["x"])) ** 2
...
class Linear(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        transpose = self.w.T
        out = torch.matmul(x, transpose)+ self.b
        # ========================
    ...
    def backward(self, dout):
        ...
        ## dout is basically derative(L)/derative(y) 
        #  
        dx = torch.matmul(dout , self.w)
        self.dw += torch.matmul(dout.T, x)
        
        ## db is the sum of the derative of the loss with respect to y
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        sigma = torch.sum(torch.exp(x), dim=1)
        loss = torch.sum(-x[torch.arange(N), y] + torch.log(sigma))
        loss = loss / N
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            mask = (torch.empty(x.size()).uniform_(0, 1) >= self.p) / (1 - self.p)
            self.grad_cache["drop"] = mask
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = dout
        last_drop = self.grad_cache["drop"]
        if self.training_mode:
            dx = torch.mul(dout , (last_drop))
        # ========================
